...
function Complete(props) {
    ...
    const getData = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                setCompleted(response.data["txn_data"]["finished"]);
    ...
    const complete = async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.data["success"]) {
                ...
                const tx = await contract.finishEscrow(
                    Number(id),
                );
    ...
    return (
        <div className="relative m-auto flex h-screen">
            <div className='relative m-auto p-2.5 pb-8 h-5/6 rounded shadow-md w-1/3 bg-slate-100'>
                <span className="text-xl mb-2 font-bold block">Accept an Escrow Agreement</span>
                <span className="text-md mb-4 block">Click on the "Accept" Button to accept the escrow agreement</span>
                <br />
                <div className={`relative z-0 w-full mb-6 group h-fit p-4 pt-2 rounded-lg ${completed ? "bg-green-200" : "bg-white"}`}>
                    <span className="text-xl font-semibold"> Status: &nbsp;  &nbsp; {completed ? "Contract Accepted" : "Pending"} </span>
                    {completed ? null :
